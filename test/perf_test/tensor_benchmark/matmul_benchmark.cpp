...
TYPED_TEST(MatmulBenchmark, PerfBenchmark) {
    ...
    for (auto _ : mdspan_prof) { moke::ops::matmul_indexing_mdspan<dtype>(a, b, d, M, N, K); }
    auto time_baseline = baseline_prof.elapsed(std::micro{});
    auto time_mdspan = mdspan_prof.elapsed(std::micro{});
    auto time_tensor = tensor_prof.elapsed(std::micro{});

    auto perf_baseline = baseline_prof.perf(size, std::giga{});
    auto perf_mdspan = mdspan_prof.perf(size, std::giga{});
    auto perf_tensor = tensor_prof.perf(size, std::giga{});

    auto rate_baseline = perf_tensor / perf_baseline * 100;
    auto rate_mdspan = perf_tensor / perf_mdspan * 100;

    std::printf("=== matmul_benchmark (M,N,K = %d,%d,%d) ===\n", M, N, K);
    std::printf("                          %8s %8s %8s\n", "baseline", "mdspan", "tensor");
    std::printf("        elapsed time(us): %8lg %8lg %8lg\n", time_baseline, time_mdspan, time_tensor);
    std::printf("    compute perf(GFLOPS): %8lg %8lg %8lg\n", perf_baseline, perf_mdspan, perf_tensor);
    std::printf("            perf rate(%%): %8lg %8lg\n", rate_baseline, rate_mdspan);
    EXPECT_TRUE(rate_baseline >= 99.9 || rate_mdspan >= 100);

    if (rate_mdspan < 100 && rate_baseline < 100) { n_fallbacks++; }
