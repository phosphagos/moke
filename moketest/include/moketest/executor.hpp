...
public:
    ...
    void execute() {
        ...
        // accuracy output
        std::printf("[ACCU] === Accuracy Comparison ===\n");
        compare_accuracy();
        std::printf("\n");
        ...
        std::printf("[PERF] === Device Performance ===\n");
        device_perf.print();
        std::printf("[PERF] === Host Performance ===\n");
        host_perf.print();
        std::printf("[PERF] === Performance Ratio ===\n");
        std::printf("[PERF] Acceleration %s\n", perf_pass ? "PASS" : "FAILED");
        std::printf("[PERF] Acceleration Ratio: %.3lfx\n", host_perf.kernel_time / device_perf.kernel_time);
    }
